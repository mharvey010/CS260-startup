...
- [x] **Components** - Multiple items edited in all jsx files in order to be compatible with jsx react. Proper references to CSS files made as well.
...
- [x] **Node.js/Express HTTP service** - Express has been implemented and the website runs on node.js when debugging.
- [x] **Static middleware for frontend** - This part has been completed.
- [x] **Calls to third party endpoints** - Gets my personal discord presence, whether I'm online or not.
- [x] **Backend service endpoints** - Many endpoints created for user login and creation as well as score tracking.
- [x] **Frontend calls service endpoints** - I use fetch requests to the service endpoints implemented on the backend.
- [x] **Supports registration, login, logout, and restricted endpoint** - Users have secure way to log in and log out of their account. Cannot create duplicate accounts.
...
- [x] **Stores credentials in MongoDB** - User login info is secure and stored in the user database. Application correctly post and get information as needed and dynamically display it as needed.

## 🚀 WebSocket deliverable

For this deliverable I did the following. I checked the box `[x]` and added a description for things I completed.

- [x] **Backend listens for WebSocket connection** - Added the peer proxy file which listens in for any web socket communications
- [x] **Frontend makes WebSocket connection** - live update notifier handles connecting to websocket
- [x] **Data sent over WebSocket connection** - Whenever a game is finished and a score is saved, data is sent over websocket and updated on all other users sides.
- [x] **WebSocket data displayed** - Data is displayed on the scores page where the live update board resides.
- [x] **Application is fully functional** - Every single part of the website is functional and can be interacted with.
